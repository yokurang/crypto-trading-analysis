...
1. **Predicting Profit and Loss (PnL)**  
We tested several models to see if we could accurately predict when the trading strategy would make or lose money. These included a few advanced machine learning methods like Gradient Boosted Machines (GBM), Convolutional Neural Networks (CNN), and H2O’s AutoML platform, as well as a simple multiple linear regression. The GBM model gave the best results, predicting profit and loss with around 12% average error. However, even the best model struggled to be consistently reliable, showing that predicting profits in high-speed crypto markets is still a tough problem. This highlights the need for better features and possibly new types of models to improve performance.

2. **Understanding Trade Direction (Buy or Sell)**  
We were able to perfectly predict whether the next trade would be a buy or sell using a simple logistic regression model, driven entirely by one key input: how far the strategy's current position is from its recent average. This aligns with the idea that market makers aim to stay balanced and manage risk. However, relying on a single variable (`deviation_from_mean_balance_diff`) makes interpretation difficult — does the strategy really ignore all other market signals? To gain deeper insight, future work should incorporate additional features like short-term price volatility, order book depth, or recent trade flow. This would help build more accurate and interpretable models that better reflect the true decision-making process.

3. **Do the Trades Move the Market? (Market Impact)**  
To test whether our trading strategy affects prices, we created a custom metric: the one-minute price change after each trade (or “fill”). Using permutation testing, we found no consistent evidence that either Maker trades (which provide liquidity) or Taker trades (which consume it) moved the market. This result supports the **Efficient Market Hypothesis (EMH)**, which suggests that asset prices already reflect all available information, so individual trades shouldn’t cause predictable price changes. Our findings align with this theory and suggest that our metric could be useful for evaluating market impact in future studies.  
*Note: Providing liquidity (Maker) means placing limit orders that wait to be filled, helping others trade. Taking liquidity (Taker) means placing market orders that are filled immediately, removing available offers from the order book.*

## Contribution to the Existing Discussion

Our study helps confirm two important ideas that professionals in the trading industry often debate:

1. Even though machine learning tools are powerful, they're not magic — predicting profit and loss in a fast-moving, competitive environment like crypto still requires deep understanding of the market and careful design of the inputs (features) used in the models.

2. Some critics worry that automated trading might unfairly influence prices. But our results suggest the opposite: this kind of trading doesn't create major price shifts and may actually help the market by making it more efficient and stable.

## Call to Action

For anyone working to improve trading strategies, especially market making, the key takeaway is to focus on the quality of data rather than relying solely on increasingly complex machine learning models. Incorporating richer, real time information such as order submission timestamps, order book depth, or macroeconomic indicators like interest rates can lead to better predictions of profit and trading behavior. In many cases, thoughtful **feature engineering** by designing meaningful inputs for models is more effective than using extremely technical models with limited interpretability.

## Limitations and Future Directions

1. Limited Data: We only analyzed trades from one strategy on one exchange, which limits our understanding of how broader market conditions — such as global events, macroeconomic indicators, or the actions of other market participants — influence performance. Having access to more diverse data and incorporating these external features could improve model accuracy and help uncover deeper insights into what drives profit and loss.

2. Model Performance: While GBM performed best in this project, other models like CNN and AutoML underperformed. Although we emphasized the importance of feature engineering over model complexity, once richer and more informative features are available, it may be worthwhile to experiment with a wider range of models — including architectures like LSTMs that capture time-based patterns — to identify which approaches deliver the most robust results.

3. Deeper Look at Market Impact: A more detailed analysis — such as checking how trades ripple through multiple exchanges — could give a clearer picture of whether this strategy influences the wider market.

In summary, this project showed that while it's possible to understand some parts of a market-making strategy, like predicting trade direction, other parts, like forecasting profits, remain much harder. Importantly, our findings suggest that this strategy helps markets function smoothly, rather than disrupt them. For professionals and researchers, the main takeaway is to focus on the quality of your data and thoughtful feature engineering rather than spending excessive effort on tuning or using highly advanced machine learning models. Only once strong, informative features are in place is it worthwhile to experiment with more complex models to see if they offer meaningful improvements.
...
The `fills_data` dataset has `1123` rows and `13` columns. The key fields include `timestamp` which shows the date-time of the trade, `order_id` which identifies each trade, `side` which shows whether a trade is "B" for buy or "S" for sell, `fill_prc` which shows what price the trade was fulfilled, `fill_qty` which shows the quantity traded, `fee` which shows the fees paid for the trade, `balance` which shows the inventory balance after the trade (in the asset or quote currency), and other fields like `symbol`, `exch`, etc., as indicated by the structure output.

The `market_data` dataset has `1208954` rows (covering the same time span with higher frequency) and 4 columns. They are `timestamp`, which is the date-time of the market snapshot, `bid_prc` which is the best bid price at that time, `ask_pr` which is the best ask price at that time, and `symbol` which is only `ETHUSDT`. For context, the bid price is the price at which people are willing to buy the asset on the market, and ask price is the price at which people are willing to sell the asset on the market. In trading, there can be many bid and ask prices, but there can be only one best bid and best ask price. The best bid price is the highest price someone is willing to purchase the asset, and the best asking price is the lowest price someone is willing to sell the asset. We assume that the `bid_prc` and `ask_prc` are the best bid and best ask prices. 
...
ggplot(merged_data, aes(x = timestamp, y = cumulative_pnl)) + 
  ...
  labs(title = "Cumulative PnL Over Time",
       x = "Timestamp",
       y = "Cumulative PnL (US$)") +
  theme_minimal()
```
...
I begin by engineering features that may help us with our modelling.

```{r}
classification.data <- merged_data
colnames(classification.data)
```

First, I will apply one-hot encoding to the categorical variables `side` and `liquidity`. 
...
table(classification.data$side)  # Buy = 1, Sell = 0, verified correct
table(classification.data$liquidity_Taker)
```
```{r}
...
```{r}
num_features <- c("volatility_lag1",
                  "spread_lag1",
                  "fill_prc",
                  "fill_qty",
                  "trade_pnl_lag1",
...
Finally, I will split the dataset into training and testing sets to assess how well the model generalizes to unseen data, which is crucial for evaluating its real-world predictive performance.

```{r}
trainIndex <- createDataPartition(scaled_data$side, p = 0.8
                                  , list = FALSE)
...
ggplot(lasso_df, aes(x = reorder(Feature, Coefficient), y = Coefficient)) +
  ...
  coord_flip() +
  labs(title = "LASSO Logistic Regression Feature Importance",
       x = "Feature", y = "Coefficient") +
  theme_minimal()

final_logistic_features <- c(selected_logistic_features, "liquidity_Taker")
final_logistic_features
```
...
for (feature in final_logistic_features) {
  p <- ggplot(logistic_pred_data, aes_string(x = feature, y = "logit")) +
    ...
    labs(title = paste("Logit vs", feature),
...
logit_model <- train(
  side ~ volatility_lag1 + spread_lag1 + fill_size + deviation_from_mean_balance_diff_lag + liquidity_Taker,
  data = train_data,
  method = "glm",
  family = binomial,
  metric = "Accuracy",
...
```{r}
# training
x_train <- model.matrix(~ volatility_lag1 + spread_lag1 + fill_size
                        + deviation_from_mean_balance_diff_lag
                        + liquidity_Taker, data = train_data)
y_train <- as.numeric(as.character(train_data$side))

# testing
x_test  <- model.matrix(~ volatility_lag1 + spread_lag1 + fill_size
                        + deviation_from_mean_balance_diff_lag
                        + liquidity_Taker, data = test_data)
...
Compared to the logistic regression model, the Ridge regression shrunk the coefficients towards zero while preserving their signs, with `deviation_from_mean_balance_diff_lag` remaining the dominant positive predictor of buy trades. Other features like `fill_size` and `liquidity_Taker` still show moderate negative influence, while `volatility_lag1` and `spread_lag1` retain small positive contributions, indicating that Ridge stabilizes but does not drastically alter the model's interpretation.
...
roc_ridge <- roc(
  response = test_data$side,
  predictor = as.numeric(pred_probs) # Ridge predicted probabilities
)
...
# if prob of side = 1 < threshold -> side = 0 (Sell)
...
# confusion Matrix (with '0' as the positive class)
...
ggplot(gbm_importance, aes(x = reorder(var, rel.inf), y = rel.inf)) +
  geom_col(fill = "steelblue") +
  coord_flip() +
  labs(title = "GBM Feature Importance",
...
```{r}
reduced_features <- c("deviation_from_mean_balance_diff_lag", 
                      "trade_pnl_lag1", 
                      "volatility_lag1", 
                      "fill_size",
                      "mid_price_lag1")
...
ggplot(importance_reduced, aes(x = reorder(var, rel.inf), y = rel.inf)) +
  geom_col(fill = "steelblue") +
  coord_flip() +
  labs(title = "Reduced GBM Feature Importance",
       x = "Feature", y = "Relative Influence") +
  theme_minimal()
...
model_names <- c("Logistic", "Ridge", "GBM", "Random Forest")
...
features_to_test <- c("bid_prc", "ask_prc", "fill_prc", "fill_qty", 
...
ggplot(na.omit(regression.data), aes(x = spread, y = trade_pnl_diff)) + 
  ...
  labs(title = "Trade PnL Diff vs. Spread",
       x = "Spread (US$)",
       y = "Trade PnL Diff (US$)") + 
  theme_minimal()
```
...
ggplot(data = na.omit(regression.data), aes(x = fee, y = trade_pnl_diff)) + 
  geom_point(color = "blue") + 
  geom_smooth(method = "lm", color = "red", se = FALSE) + 
  labs(title = "Trade PnL Diff vs. Fees",
       x = "Fees (US$)",
       y = "Trade PnL Diff (US$)") + 
  theme_minimal()
```
...
ggplot(na.omit(regression.data), aes(x = fill_size, y = trade_pnl_diff)) + 
  geom_line(color = "blue") + 
  geom_smooth(method = "lm", color = "red", se = FALSE) +
  labs(title = "Trade PnL Diff vs. Fill Size",
       x = "Fill Size (US$)",
       y = "Trade PnL Diff (US$)") + 
  theme_minimal()
```
...
cat("ADF Test for Deviation from Deviation from Mean Inventory Balance - p-value:",
...
ggplot(na.omit(regression.data), aes(x = deviation_from_mean_balance, y = trade_pnl_diff)) + 
  geom_line(color = "blue") + 
  geom_smooth(method = "lm", color = "red", se = FALSE) +
  labs(title = "Trade PnL Diff vs. Deviation from Mean Inventory Balance",
...
cat("ADF Test for Deviation from Deviation from Mean Mid Price - p-value:",
...
adf_deviation_from_mean_mid_price_diff <- adf.test(
  na.omit(regression.data$deviation_from_mean_mid_price_diff), alternative = "stationary"
  )
...
ggplot(na.omit(regression.data), aes(x = deviation_from_mean_mid_price_diff, y = trade_pnl_diff)) + 
  geom_line(color = "blue") + 
  geom_smooth(method = "lm", color = "red", se = FALSE) +
  labs(title = "Trade PnL Diff vs. Deviation from Mean Mid Price Diff",
...
ggplot(na.omit(regression.data), aes(x = volatility, y = trade_pnl_diff)) + 
  geom_point(color = "blue") + 
  geom_smooth(method = "lm", color = "red", se = FALSE) +
  labs(title = "Trade PnL Diff vs. Volatility",
...
features = c("fee", "spread", "fill_size", "deviation_from_mean_balance",
...
alias(lm(as.formula(paste("trade_pnl_diff ~", paste(features, collapse=" + ")))
         , data = scaled_data_encoded))$Complete
```
...
```{r}
vif_formula <- as.formula(paste("trade_pnl_diff ~",
...
for (feature in numerical_best_features) {
  print(
    ggplot(scaled_data_encoded, aes_string(x = feature, y = "trade_pnl_diff")) + 
      ...
      geom_smooth(method = "lm", color = "red", se = FALSE) +  
      labs(title = paste("Trade PnL Diff vs.", feature),
...
```{r}
mlr.fit <- lm(as.formula(paste("trade_pnl_diff", "~",
             paste(best_vif_features, collapse=" + "))),
             data = train_data) # use training set
summary(mlr.fit)
```
...
mlr_metrics_df <- tibble(
  ...
  Value = c(test_metrics$RMSE, oob_metrics$RMSE,
...
subset_metrics_df <- tibble(
  Metric = c("RMSE", "OOB RMSE", "MAE", "OOB MAE", "MAPE", "Adjusted R-Squared"),
  Value = c(test_metrics_subset$RMSE, oob_metrics_subset$RMSE,
...
ggplot(combined_varimp, aes(x = reorder(Feature, Overall), y = Overall, fill = Model)) +
  ...
  coord_flip() +
  labs(title = "Variable Importance: MLR vs Best Subset",
...
```{r}
library(caret)
rfe_ctrl <- rfeControl(functions = caretFuncs, # GBM
                       method = "cv",
                       number = 5) # computation-accuracy trade off

rfe_result <- rfe(x = scaled_data_encoded[, features],
...
gbm_formula <- as.formula(paste("trade_pnl_diff", "~",
...
ggplot(gbm_importance, aes(x = reorder(var, rel.inf), y = rel.inf)) +
  geom_col(fill = "steelblue") +
  coord_flip() +
  labs(title = "Feature Importance (GBM)",
       x = "Feature", y = "Relative Influence") +
  theme_minimal()
```
...
ggplot(filter(gbm_metrics_df, Metric != "MAPE"), 
...
h2o_results <- data.frame(Max_Models = integer(), RMSE = numeric(),
...
for (max_mod in model_counts) {
  ...
  h2o_results <- rbind(h2o_results,
                       data.frame(Max_Models = max_mod, RMSE = aml_rmse,
...
rfe_ctrl <- rfeControl(functions = rfFuncs, # Use random forest specific RFE
                       method = "cv",
                       number = 10)

rfe_result <- rfe(x = train_data[, features], # start with features
...
ggplot(filter(rf_metrics_df, Metric != "MAPE"), 
       aes(x = reorder(Metric, -Value), y = Value, fill = Metric)) +
  geom_bar(stat = "identity") +
  labs(title = "Random Forest Performance Metrics (Test Set)",
       x = "Metric", y = "Value") +
  theme_minimal() +
  theme(legend.position = "none")
...
train_cnn <- function(filters, kernel_sz, hidden_size, lr, epochs) {
  ...
  if (conv_out_dim <= 0) {
    warning(paste("Skipping invalid combo: kernel_sz =", kernel_sz, 
...
for (filters in filter_grid) {
  for (kernel_sz in kernel_grid) {
    for (hidden_size in hidden_grid) {
      for (lr in lr_grid) {
        cat("Training CNN → filters:", filters,
            "| kernel:", kernel_sz,
            "| hidden:", hidden_size,
            "| lr:", lr, "\n")
...
ggplot(non_mape_melted, aes(x = Metric, y = value, fill = variable)) +
  ...
  geom_text(aes(label = round(value, 4)),
            position = position_dodge(width = 0.9),
            vjust = -0.3, size = 3) +
  labs(title = "Model Performance Comparison (excluding MAPE)",
       x = "Metric", y = "Value") +
  theme_minimal() +
...
ggplot(mape_melted, aes(x = variable, y = value, fill = variable)) +
  ...
  labs(title = "MAPE Comparison Across Models",
       x = "Model", y = "MAPE (%)") +
  theme_minimal() +
  theme(legend.position = "none")
```
...
ggplot(best_subset_varimp, aes(x = reorder(Feature, Overall), y = Overall)) +
  geom_col(fill = "steelblue") +
  coord_flip() +
  labs(title = "Feature Importance (Best Subset Linear Model)",
...
permutation_test_one_sample <- function(x, R = 10000) {
  ...
  for (i in 1:R) {
    perm_stats[i] <- mean(sample(x))
  }
  
  two_sided_p <- (sum(abs(perm_stats) >= abs(obs_stat)) + 1) / (R + 1)
  
  ci <- quantile(perm_stats, c(0.025, 0.975))
...
Overall, the permutation tests suggest no statistically significant deviation from zero in either the Maker or Taker subsets, for both absolute and percentage price changes:

1. Maker Trades
   - The observed mean (`price_change = 0.525`, `price_change_pct ≈ 0.00028`) is nonzero, but the p-values (~0.998 and ~0.9945) are extremely high.  
   - The 95% permutation confidence intervals are degenerate at the observed means (both lower and upper bounds are the same number), indicating the permutation distribution did not vary under this particular dataset and code run.  
   - Interpreted together, this implies there is no evidence that Maker trades systematically shift prices either up or down.

2. Taker Trades**
   - The observed mean (`price_change = -0.406`, `price_change_pct ≈ -0.00021`) is slightly negative, but again the two-sided p-values (~0.58 and ~0.53) show no significant difference from zero.  
   - The degenerate 95% CIs mirror the observed means, suggesting the permutation draws yielded no variation for the test statistic.  
   - Consequently, Taker trades also show no statistically robust post-trade price movement away from zero.

Because the permutation tests fail to reject the null hypothesis in every case (large p-values), the data here do not support the claim that trading—whether Maker or Taker—causes a lasting or systematic price impact. Instead, the results align with the idea that any observed post-trade price changes could be due to random fluctuations rather than a true directional shift in market prices.

In the context of the Efficient Market Hypothesis (EMH), these findings indicate that the market appears to behave efficiently with respect to the trading strategy under investigation. Prices seem to incorporate available information rapidly, and there is no statistical evidence that trades result in predictable or persistent price movements.

# Final Conclusion

Overall, the project demonstrates that while the classification of Buy versus Sell trades achieves near-perfect performance, particularly when incorporating `deviation_from_mean_balance_diff` as a key feature, developing a robust model for Profit and Loss (PnL) prediction remains challenging. The Gradient Boosting Model (GBM) emerges as the strongest among the approaches attempted for forecasting PnL, but further improvements in feature engineering and model tuning are needed, especially for more complex methods such as CNNs and H2O AutoML. Additionally, hypothesis testing based on post-trade price changes suggests that the strategy does not cause a lasting or systematic market impact. Instead, the data are consistent with the Efficient Market Hypothesis (EMH), indicating that prices rapidly absorb the information conveyed by these trades without a predictable, directionally biased shift.
